---
template: Shopify Order Refund
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Refund notification
source-file: aydins-order-refund-shopify.html
version: V3 (Patch A applied 2026-05-16, product images render)
---

# Paste-Ready Code: Order Refund

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Refund notification** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Order refund notification template
...
<body>
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        <h1>Your refund was processed.</h1>
        <p>Hi {{ customer_name }},</p>
        <p>We processed a refund for Aydins order {{ order_label }}.</p>
        <p>The refund is sent back to the original payment method. Depending on your bank or card issuer, it may take a few business days to show on your statement.</p>
        ...
        <div class="panel">
          ...
          {% if refund_line_items %}
            ...
            {% endfor %}
          {% endif %}
          {% if amount %}<p><strong>Refund amount:</strong> {{ amount | money_with_currency }}</p>{% endif %}
          <p class="muted">Refunds are sent back to the original payment method. Your bank or card issuer controls final posting time.</p>
        </div>
        <div class="panel">
          <h2>Need help?</h2>
          <p>If this refund does not look right, reply to this email with your order number. Our team will review it.</p>
        </div>
