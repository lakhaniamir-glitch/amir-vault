---
template: Shopify Return Request Declined
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Return request declined
source-file: aydins-return-request-declined-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Return Request Declined

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Return request declined** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Return request declined notification template
...
<head>
  ...
  <title>About your return request: {{ shop.name }}</title>
  <!--[if mso]>
...
<body>
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        <h1>About your return request.</h1>
        <p>Hi {{ customer_name }},</p>
        <p>We reviewed your return request for Aydins order {{ order_label }}. We are not able to approve this return.</p>

        {% if return.decline_note != blank %}
          <div class="panel">
            <h2>Reason</h2>
            <p>{{ return.decline_note | newline_to_br }}</p>
          </div>
        {% elsif return.decline_reason %}
          <div class="panel">
            <h2>Reason</h2>
            <p>{{ return.decline_reason }}</p>
          </div>
        {% endif %}

        <div class="panel">
          <h2>How we can still help</h2>
          <p>If this is a sizing or fit issue, reply to this email and let us know. We offer exchanges in many cases where a return is not possible, especially on engraved pieces.</p>
          <p>If there is something wrong with the ring itself (workmanship, finish, or material), the Aydins Lifetime Warranty may cover it. Reply with photos and details and we will review.</p>
        </div>
        ...
        <p>We want you wearing a ring you actually love. Reply and let us figure out the next step together.</p>
      </div>
