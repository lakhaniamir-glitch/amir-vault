---
template: Shopify Customer Email Address Change Confirmation
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Customer email address change confirmation
source-file: aydins-customer-email-change-confirmation-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Customer Email Address Change Confirmation

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Customer email address change confirmation** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Customer email address change confirmation notification template
...
<head>
  ...
  <title>Confirm your new email: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <style>
    ...
    .email-block { font-family: Georgia, 'Times New Roman', serif; font-size:18px; color:#1d1a17; margin:0; word-break:break-all; }
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>We got a request to change the email address on your Aydins account. Confirm the new address below to complete the change.</p>

        {% if customer.new_email %}
          <div class="panel">
            <h2>New email address</h2>
            <p class="email-block">{{ customer.new_email }}</p>
          </div>
        {% endif %}

        {% if customer.verify_email_url %}
          <p><a class="button" href="{{ customer.verify_email_url }}">Confirm new email</a></p>
        {% elsif verify_email_url %}
          <p><a class="button" href="{{ verify_email_url }}">Confirm new email</a></p>
        {% endif %}

        <div class="panel">
          <h2>Heads up</h2>
          <p>Until you confirm, your account email stays the same. If you did not request this change, ignore this email and your account stays untouched.</p>
        </div>

        <p class="muted">Questions? Reply to this email. Our team will sort it out.</p>
      </div>
