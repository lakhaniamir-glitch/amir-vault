---
template: Shopify Customer Payment Method Update Request
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Customer payment method update request
source-file: aydins-customer-payment-method-update-request-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Customer Payment Method Update Request

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Customer payment method update request** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Customer payment method update request notification template
...
<head>
  ...
  <title>Update your payment method: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>The card saved to your Aydins account needs an update. Use the secure link below to refresh your payment details.</p>

        {% if payment_method_url %}
          <p><a class="button" href="{{ payment_method_url }}">Update payment method</a></p>
        {% endif %}

        <div class="panel">
          <h2>Why you got this</h2>
          <p>This usually means a card on file has expired, was declined, or needs to be re-verified. Updating now keeps future Aydins orders from getting stuck at checkout.</p>
        </div>

        <p>If you did not request this, you can ignore the email. No charges will be made without your approval.</p>
      </div>
