---
template: Shopify Draft Order Invoice
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Draft order invoice
source-file: aydins-draft-order-invoice-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Draft Order Invoice

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Draft order invoice** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Draft order invoice notification template
...
{% assign order_label = name | default: order_name %}
...
<head>
  ...
  <title>Your Aydins invoice: {{ shop.name }}</title>
  <!--[if mso]>
...
<body>
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        <h1>Your Aydins invoice.</h1>
        <p>Hi {{ customer_name }},</p>
        <p>Here is the invoice for the order we put together for you{% if order_label %} ({{ order_label }}){% endif %}. Review the details and complete payment using the button below.</p>

        {% if invoice_url %}
          <p><a class="button" href="{{ invoice_url }}">Complete payment</a></p>
        {% endif %}

        {% if custom_message != blank %}
          <div class="panel">
            <h2>A note from Aydins</h2>
            <p style="white-space:pre-wrap;">{{ custom_message }}</p>
          </div>
        {% endif %}
        ...
        <p>Reply with any questions before paying. We will hold the order open on our side until you are ready.</p>
      </div>
