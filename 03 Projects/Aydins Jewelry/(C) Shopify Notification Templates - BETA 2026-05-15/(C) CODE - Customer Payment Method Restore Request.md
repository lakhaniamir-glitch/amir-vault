---
template: Shopify Customer Payment Method Restore Request
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Customer payment method restore request
source-file: aydins-customer-payment-method-restore-request-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Customer Payment Method Restore Request

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Customer payment method restore request** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Customer payment method restore request notification template
...
<head>
  ...
  <title>Restore your payment method: {{ shop.name }}</title>
  <!--[if mso]>
...
<body>
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        <h1>Restore your payment method.</h1>
        <p>Hi {{ customer_name }},</p>
        <p>A previously removed payment method on your Aydins account is being restored. Use the secure link below to confirm and re-authorize the card.</p>

        {% if payment_method_url %}
          <p><a class="button" href="{{ payment_method_url }}">Restore payment method</a></p>
        {% endif %}

        <div class="panel">
          <h2>What this means</h2>
          <p>Restoring re-enables a card you previously had on file for use on future Aydins orders. Nothing will be charged until you authorize a new purchase.</p>
        </div>

        <p>If you did not request this, you can ignore the email. The card will not be restored unless you confirm.</p>
      </div>
