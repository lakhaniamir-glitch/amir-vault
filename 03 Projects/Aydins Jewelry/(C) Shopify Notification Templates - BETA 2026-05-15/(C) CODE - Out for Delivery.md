---
template: Shopify Out for Delivery
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Out for delivery
source-file: aydins-out-for-delivery-shopify.html
version: V3 (Patch A applied 2026-05-16, product images render)
---

# Paste-Ready Code: Out for Delivery

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Out for delivery** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Out for delivery notification template
...
<head>
  ...
  <title>Out for delivery: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>Your Aydins order {{ order_label }} is out for delivery today.</p>
        <p>Keep an eye on the tracking page for the final scan. Your piece was prepared and packed by our team in Irving, Texas, and it is almost there.</p>

        {% if tracking_url_primary %}
          <p><a class="button" href="{{ tracking_url_primary }}">Track delivery</a></p>
        ...
        {% endif %}
        <p>If the carrier marks it delivered and you do not see it, reply to this email. We will help you check the next steps.</p>
