---
template: Shopify Return Request Approved
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Return request approved
source-file: aydins-return-request-approved-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Return Request Approved

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Return request approved** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Return request approved notification template
...
<head>
  ...
  <title>Return approved: {{ shop.name }}</title>
  <!--[if mso]>
...
<body>
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        <h1>Your return is approved.</h1>
        <p>Hi {{ customer_name }},</p>
        <p>Your return request for Aydins order {{ order_label }} is approved. Here is what to do next.</p>

        <div class="panel">
          <h2>Next steps</h2>
          <p>Pack the ring securely in its original box. If a return shipping label was issued, it is linked below. Otherwise, follow the shipping instructions sent separately. Once we receive and inspect the piece, we will issue your refund or credit.</p>
        </div>

        {% if return.return_shipping_labels %}
          {% for label in return.return_shipping_labels %}
            {% if label.url %}
              <p><a class="button" href="{{ label.url }}">Download shipping label</a></p>
            {% endif %}
          {% endfor %}
        ...
        {% if return.return_line_items %}
          <div class="panel">
            <h2>Items approved for return</h2>
        ...
        <p class="muted">A restocking fee may apply per our return policy. Final refund amount will be confirmed in your refund email once we inspect the items.</p>
        <p>Reply to this email with any questions before you ship.</p>
      </div>
