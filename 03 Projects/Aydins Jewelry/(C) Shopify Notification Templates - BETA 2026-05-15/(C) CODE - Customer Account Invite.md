---
template: Shopify Customer Account Invite
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Customer account invite
source-file: aydins-customer-account-invite-shopify.html
version: V2 (built 2026-05-17, custom_message bare-drop patch 2026-05-17)
patch-notes: V2 adds bare {{ custom_message }} drop so Shopify's "missing drop" validator detects it. Personal notes from the admin "Send account invite" dialog render in the email body.
---

# Paste-Ready Code: Customer Account Invite

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Customer account invite** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Customer account invite notification template
...
<head>
  ...
  <title>Activate your Aydins account: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>We have set up an Aydins account for you. Activate it to view orders, track shipments, and reach us faster on future purchases.</p>
        ...
        {% if customer.account_activation_url %}
          <p><a class="button" href="{{ customer.account_activation_url }}">Activate account</a></p>
        {% elsif account_activation_url %}
          <p><a class="button" href="{{ account_activation_url }}">Activate account</a></p>
        {% endif %}

        <div class="panel">
          <h2>Why activate it</h2>
          <p>Your account is the cleanest way to keep tabs on Aydins orders, especially if you come back for another ring, sizing help, or engraving support later.</p>
        </div>

        <p>If you did not expect this email, you can ignore it. Your account will not activate unless you use the link above.</p>
      </div>
