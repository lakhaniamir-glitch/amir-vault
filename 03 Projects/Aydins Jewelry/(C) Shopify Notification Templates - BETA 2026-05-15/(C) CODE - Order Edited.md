---
template: Shopify Order Edited
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Order edited
source-file: aydins-order-edited-shopify.html
version: V3 (Patch A applied 2026-05-16, product images render)
---

# Paste-Ready Code: Order Edited

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Order edited** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Order edited notification template
...
<head>
  ...
  <title>Order updated: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>Order {{ order_label }} has been updated.</p>
        <p>If you requested the change, you are all set. If anything looks different than expected, reply to this email as soon as possible so we can review it before the order moves further through the shop.</p>

        {% if order_status_url %}
          <p><a class="button" href="{{ order_status_url }}">View updated order</a></p>
        {% endif %}
        ...
        {% endif %}
        <div class="panel">
          <h2>Quick note from the shop</h2>
          <p>For engraved pieces, timing matters. If the edit touches size, engraving, finish, or shipping address, reply quickly so our Irving, Texas team can catch it before production or shipping.</p>
        </div>
