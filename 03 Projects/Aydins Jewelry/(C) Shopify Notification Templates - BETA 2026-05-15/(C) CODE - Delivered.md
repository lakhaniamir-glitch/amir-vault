---
template: Shopify Delivered
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Delivered
source-file: aydins-delivered-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Delivered

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Delivered** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Delivered notification template
...
<head>
  ...
  <title>Delivered: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>Your Aydins order {{ order_label }} was marked delivered by the carrier today.</p>
        <p>We hope the piece looks even better in person than it did on the screen.</p>

        {% if tracking_url_primary %}
          <p><a class="button" href="{{ tracking_url_primary }}">View delivery confirmation</a></p>
        {% elsif order_status_url %}
          <p><a class="button" href="{{ order_status_url }}">View order</a></p>
        {% endif %}

        <div class="panel">
          <h2>Backed by Aydins</h2>
          <p>Every Aydins ring is covered by the Aydins Lifetime Warranty against manufacturing defects. If you ever need help with sizing, care, or anything else, reply to this email and a real person will help.</p>
        </div>
        ...
        <p>If the carrier marked it delivered but you do not have it, reply to this email right away. We will help you check the next steps.</p>
      </div>
