---
template: Shopify Order Payment Receipt
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Order payment receipt
source-file: aydins-order-payment-receipt-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Order Payment Receipt

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Order payment receipt** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Order payment receipt notification template
...
<head>
  ...
  <title>Payment received: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>We received your payment for order {{ order_label }}. Your piece is moving into our production queue.</p>
        ...
        <div class="panel">
          ...
          {% if transactions %}
            {% for transaction in transactions %}
              {% if transaction.kind == 'authorization' or transaction.kind == 'sale' or transaction.kind == 'capture' %}
                {% if transaction.status == 'success' %}
                  {% if transaction.payment_details.credit_card_company %}
                    <p style="margin:0 0 6px;">{{ transaction.payment_details.credit_card_company }} ending in {{ transaction.payment_details.credit_card_last_four_digits }}</p>
                  {% elsif transaction.gateway %}
                    <p style="margin:0 0 6px;">{{ transaction.gateway | replace: '_', ' ' | capitalize }}</p>
                  {% endif %}
                  {% if transaction.amount %}<p style="margin:0;font-size:14px;color:#4c453e;">Amount: {{ transaction.amount | money_with_currency }}</p>{% endif %}
                {% endif %}
              {% endif %}
            {% endfor %}
          {% elsif total_price %}
            <p style="margin:0;">Total paid: {{ total_price | money_with_currency }}</p>
          {% endif %}
        </div>

        <p>Shipping confirmation will follow once your piece is on its way. Reply if anything looks off.</p>
      </div>
