---
template: Shopify Contact Customer
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Contact customer
source-file: aydins-contact-customer-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Contact Customer

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Contact customer** template, then **Save**. This template wraps whatever message you type in the Shopify admin when you click **Email customer** on an order. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Contact customer notification template
Use in Shopify Admin > Settings > Notifications > Customer notifications.
This wraps the custom message you type when emailing a customer from the order page.
...
<head>
  ...
  <title>A message from Aydins: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <style>
    ...
    .message-body { font-size:15px; line-height:1.7; color:#2b2723; }
...
<body>
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        <h1>A note from Aydins.</h1>
        <p>Hi {{ customer_name }},</p>

        {% if custom_message %}
          <div class="message-body" style="white-space:pre-wrap;">{{ custom_message }}</div>
        {% endif %}

        {% if order_status_url %}
          <p style="margin-top:24px;"><a class="button" href="{{ order_status_url }}">View order {{ order_label }}</a></p>
        {% endif %}

        <p class="muted" style="margin-top:18px;">Reply directly to this email and it comes straight to our team.</p>
      </div>
