---
template: Shopify Pending Payment Success
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Pending payment success
source-file: aydins-pending-payment-success-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Pending Payment Success

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Pending payment success** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Pending payment success notification template
...
<head>
  ...
  <title>Payment confirmed: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>Your pending payment for order {{ order_label }} cleared. Your piece is now in our production queue.</p>
        ...
        <div class="panel">
          ...
          <p>If your order includes engraving, we set it up and double-check the details before shipping. You will get a separate shipping confirmation with tracking once the package leaves our shop.</p>
        </div>
