---
template: Shopify Customer Payment Method Add Request
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Customer payment method add request
source-file: aydins-customer-payment-method-add-request-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Customer Payment Method Add Request

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Customer payment method add request** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Customer payment method add request notification template
...
<head>
  ...
  <title>Add a payment method: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>You can securely save a payment method to your Aydins account using the link below. We use it for faster checkout and authorized future charges only.</p>

        {% if payment_method_url %}
          <p><a class="button" href="{{ payment_method_url }}">Add payment method</a></p>
        {% endif %}

        <div class="panel">
          <h2>What this does</h2>
          <p>Saves a card on file securely with our payment processor. We do not store full card numbers. You can update or remove it at any time from your Aydins account.</p>
        </div>

        <p>If you did not expect this email, you can ignore it. No card will be saved unless you use the link above.</p>
      </div>
