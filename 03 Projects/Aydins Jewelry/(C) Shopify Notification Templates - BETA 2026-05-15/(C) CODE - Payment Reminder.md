---
template: Shopify Payment Reminder
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Payment reminder
source-file: aydins-payment-reminder-shopify.html
version: V3 (built 2026-05-17, custom_message bare-drop patch 2026-05-17)
patch-notes: V3 uses bare {{ custom_message }} (no Liquid filter) so Shopify's "missing drop" validator detects it. Filters cause the validator to flag the drop as missing even though it renders. Matches Shopify's default template pattern.
---

# Paste-Ready Code: Payment Reminder

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Payment reminder** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Payment reminder notification template
...
{% assign payment_url_primary = payment_collection_url | default: order_status_url %}
...
<head>
  ...
  <title>Reminder: payment due for {{ order_label }}: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>Aydins order {{ order_label }} is still waiting on payment. Once it clears, your piece moves into our production queue and starts moving toward you.</p>
        ...
        {% if payment_url_primary %}
          <p><a class="button" href="{{ payment_url_primary }}">Complete payment</a></p>
        {% endif %}

        {% if total_price %}
          <div class="panel">
            <h2>Amount due</h2>
            <p style="font-size:18px;color:#1d1a17;margin:0;"><strong>{{ total_price | money_with_currency }}</strong></p>
          </div>
        {% endif %}

        <p>If you have changed your mind or are not ready, that is fine. Just reply and let us know so we can release the hold. No pressure.</p>
      </div>
