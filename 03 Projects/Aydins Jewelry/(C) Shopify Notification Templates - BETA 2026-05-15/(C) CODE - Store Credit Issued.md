---
template: Shopify Store Credit Issued
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Store credit issued
source-file: aydins-store-credit-issued-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Store Credit Issued

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Store credit issued** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Store credit issued notification template
...
<head>
  ...
  <title>Store credit added: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <style>
    ...
    .credit-amount { font-family: Georgia, 'Times New Roman', serif; font-size:36px; color:#1d1a17; font-weight:600; letter-spacing:1px; margin:0 0 8px; }
...
<body>
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        <h1>Store credit added.</h1>
        <p>Hi {{ customer_name }},</p>
        <p>We have added store credit to your Aydins account. It is ready to use on your next order.</p>

        <div class="panel" style="text-align:center;">
          {% if store_credit_account.balance %}
            <p class="credit-amount">{{ store_credit_account.balance | money_with_currency }}</p>
            <p class="muted">Available balance</p>
          {% elsif amount %}
            <p class="credit-amount">{{ amount | money_with_currency }}</p>
            <p class="muted">Credit issued</p>
          {% endif %}
          {% if store_credit_account.expires_at %}
            <p class="muted" style="margin-top:14px;">Expires {{ store_credit_account.expires_at | date: "%B %d, %Y" }}</p>
          {% endif %}
        </div>

        <p><a class="button" href="https://shopaydins.com/collections/all">Shop wedding bands</a></p>

        <div class="panel">
          <h2>How it works</h2>
          <p>Your credit will apply automatically at checkout when you are signed in to your Aydins account. If you need help, reply to this email.</p>
        </div>
      </div>
