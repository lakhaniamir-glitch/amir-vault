---
template: Shopify Customer Password Reset
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Customer password reset
source-file: aydins-customer-password-reset-shopify.html
version: Phase 3 (built 2026-05-16, no line items so no image rule)
---

# Paste-Ready Code: Customer Password Reset

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Customer password reset** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Customer password reset notification template
...
<head>
  ...
  <title>Reset your Aydins password: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>We received a request to reset the password for your Aydins account.</p>
        <p>If that was you, use the button below to set a new password.</p>

        {% if customer.reset_password_url %}
          <p><a class="button" href="{{ customer.reset_password_url }}">Reset password</a></p>
        {% elsif reset_password_url %}
          <p><a class="button" href="{{ reset_password_url }}">Reset password</a></p>
        {% endif %}

        <div class="panel">
          <h2>If this was not you</h2>
          <p>You can ignore this email. Your password will not change unless you use the reset link.</p>
        </div>

        <p>If you need help getting back into your account, reply to this email.</p>
