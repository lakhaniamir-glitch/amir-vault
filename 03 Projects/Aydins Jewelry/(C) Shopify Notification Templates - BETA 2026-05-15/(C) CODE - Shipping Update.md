---
template: Shopify Shipping Update
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Shipping update
source-file: aydins-shipping-update-shopify.html
version: V3 (Patch A applied 2026-05-16, product images render)
---

# Paste-Ready Code: Shipping Update

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Shipping update** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Shipping update notification template
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>There is an update on Aydins order {{ order_label }}.</p>
        <p>This can happen when a carrier updates tracking, a package changes routes, an address detail is adjusted, or delivery timing changes. The latest available details are below.</p>

        {% if tracking_url_primary %}
          <p><a class="button" href="{{ tracking_url_primary }}">View latest tracking</a></p>
        ...
        <div class="panel">
          ...
          <div class="tracking-box">
            ...
            {% if tracking_url_primary %}
              <p class="muted">Carrier scans can take a few hours to appear after the package moves through the next checkpoint.</p>
            {% else %}
        ...
        {% endif %}
        <p>If the update looks wrong or the address needs attention, reply to this email right away. We will help if the package can still be redirected or corrected.</p>
