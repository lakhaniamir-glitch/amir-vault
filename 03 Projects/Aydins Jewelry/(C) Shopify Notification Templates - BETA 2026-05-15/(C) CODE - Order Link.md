---
template: Shopify Order Link
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Order link
source-file: aydins-order-link-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Order Link

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Order link** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Order link notification template
...
<head>
  ...
  <title>Your order link: {{ shop.name }}</title>
  <!--[if mso]>
...
<body>
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        <h1>Your order link.</h1>
        <p>Hi {{ customer_name }},</p>
        <p>Here is a direct link to Aydins order {{ order_label }}. Save it for quick access to tracking, details, and updates.</p>

        {% if order_status_url %}
          <p><a class="button" href="{{ order_status_url }}">View your order</a></p>
        {% endif %}

        <p class="muted">If you need anything from us, reply to this email. Real people answer.</p>
      </div>
