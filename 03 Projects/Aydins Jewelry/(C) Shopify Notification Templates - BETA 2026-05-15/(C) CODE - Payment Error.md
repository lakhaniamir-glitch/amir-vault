---
template: Shopify Payment Error
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Payment error
source-file: aydins-payment-error-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Payment Error

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Payment error** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Payment error notification template
...
<head>
  ...
  <title>Payment couldn't be processed: {{ shop.name }}</title>
  <!--[if mso]>
  ...
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        ...
        <p>We were not able to process payment for Aydins order {{ order_label }}. Your order is on hold until payment is resolved.</p>
        <p>This usually means the card was declined, expired, or had a billing-address mismatch. Updating the payment method is usually all it takes.</p>

        {% if payment_url_primary %}
          <p><a class="button" href="{{ payment_url_primary }}">Update payment</a></p>
        {% endif %}

        <div class="panel">
          <h2>Order on hold</h2>
          <p>We will hold the order on our side while you sort this out. Nothing leaves the shop until payment clears.</p>
        </div>

        <p>If you need help, reply to this email and we will walk you through it.</p>
      </div>
