---
template: Shopify Pending Payment Error
paste-into: Shopify Admin → Settings → Notifications → Customer notifications → Pending payment error
source-file: aydins-pending-payment-error-shopify.html
version: V1 (built 2026-05-17)
---

# Paste-Ready Code: Pending Payment Error

**How to use:** Copy everything inside the code block below. In Shopify, replace the entire email body of the **Pending payment error** template, then **Save**. **Preview** before going live.

```liquid
{% comment %}
Aydins Jewelry - Shopify Pending payment error notification template
...
<head>
  ...
  <title>Pending payment didn't clear: {{ shop.name }}</title>
  <!--[if mso]>
...
<body>
  <div class="wrap">
    <div class="container">
      ...
      <div class="content">
        <h1>Your pending payment didn't clear.</h1>
        <p>Hi {{ customer_name }},</p>
        <p>The pending payment on Aydins order {{ order_label }} did not go through. Your order is still on hold until payment is resolved.</p>
        <p>This sometimes happens with bank transfers, buy-now-pay-later, or manual payments when the payment is rejected before it settles.</p>

        {% if payment_url_primary %}
          <p><a class="button" href="{{ payment_url_primary }}">Retry payment</a></p>
        {% endif %}

        <div class="panel">
          <h2>What now</h2>
          <p>Try the payment again, or pick a different payment method using the link above. We will hold the order on our side until you sort this out.</p>
        </div>

        <p>Reply to this email if you need help. We can sometimes invoice you directly if your bank is being picky.</p>
      </div>
